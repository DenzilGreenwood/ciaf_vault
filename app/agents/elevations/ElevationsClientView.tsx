...
interface ElevationsClientViewProps {
  initialGrants: ElevationGrant[]
}

export default function ElevationsClientView({ initialGrants }: ElevationsClientViewProps) {
  ...
  const expiringSoon = initialGrants.filter(g => {
    const diffMs = new Date(g.valid_until).getTime() - new Date().getTime()
    return diffMs < 24 * 60 * 60 * 1000
  }).length

  const totalUses = initialGrants.reduce((sum, g) => sum + g.used_count, 0)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <div className="mt-2 text-3xl font-bold text-yellow-600 dark:text-yellow-400">{initialGrants.length}</div>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
